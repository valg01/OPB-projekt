...
SELECT
    c.confederation_code as ConfederationCode --, c.confederation_name as ConfederationName
,
    tour.host_country,
...
--povprečna starost igralcev po turnirjih in državah
...
SELECT
    ...
    COUNT(
        CASE
            WHEN "position" = 1 then ts.team_id
        END
    ) as winners,
    COUNT(
        CASE
            WHEN "position" = 2 then ts.team_id
        END
    ) as runnerups,
    COUNT(
        CASE
            WHEN "position" = 3 then ts.team_id
        END
    ) as third,
    COUNT(
        CASE
            WHEN "position" = 4 then ts.team_id
        END
    ) as fourth
